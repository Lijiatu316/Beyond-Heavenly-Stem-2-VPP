...
function results = run_mode_cooperate()
    ...
    hist.exchange_VPP12 = zeros(N, 1);
    hist.exchange_VPP23 = zeros(N, 1);
    hist.exchange_VPP31 = zeros(N, 1);
    hist.comm_delay     = zeros(N, 1);
    ...
    for t = 1:N
        ...
        hist.comm_delay(t)     = mean(comm_state.delay_matrix(:));
        if size(admm_info.exchange_power, 1) >= 3
            hist.exchange_VPP12(t) = admm_info.exchange_power(1, 2);
            hist.exchange_VPP23(t) = admm_info.exchange_power(2, 3);
            hist.exchange_VPP31(t) = admm_info.exchange_power(3, 1);
        end
...
function export_cooperate_csv(results, cfg)
    ...
    header = sprintf('%s,VPP12_Exchange_kW,VPP23_Exchange_kW,VPP31_Exchange_kW', header);
    header = sprintf('%s,ADMM_Residual,Comm_Delay_ms', header);
    ...
    for t = 1:N
        ...
        for v = 1:cfg.N_vpp
            fprintf(fid, ',%.4f', h.P_pv(t, v));
        end
        for v = 1:cfg.N_vpp
            fprintf(fid, ',%.4f', h.P_wind(t, v));
        end
        for v = 1:cfg.N_vpp
            fprintf(fid, ',%.4f', h.P_gas(t, v));
        end
        for v = 1:cfg.N_vpp
            fprintf(fid, ',%.4f', h.P_bat(t, v));
        end
        for v = 1:cfg.N_vpp
            fprintf(fid, ',%.4f', h.P_load(t, v));
        end
        for v = 1:cfg.N_vpp
            fprintf(fid, ',%.4f', h.P_shed(t, v));
        end
        for v = 1:cfg.N_vpp
            fprintf(fid, ',%.4f', h.P_curtail(t, v));
        end
        for v = 1:cfg.N_vpp
            fprintf(fid, ',%.6f', h.SOC(t, v));
        end
        for v = 1:cfg.N_vpp
            fprintf(fid, ',%.6f', h.freq(t, v));
        end
        fprintf(fid, ',%.4f,%.4f,%.4f', h.exchange_VPP12(t), h.exchange_VPP23(t), h.exchange_VPP31(t));
        fprintf(fid, ',%.6f,%.3f', h.admm_residual(t), h.comm_delay(t) * 1000);  % s→ms
        fprintf(fid, '\n');
    end

    fclose(fid);
    fprintf('  协同CSV导出: cooperate_timeseries.csv (%d行)\n', N);
end
